...
void Jatek::karakter_fajl_beolvas(){
    ...
    // Régi karakterek felszabadítása
    if(this->karakterek_szama>0){ delete[] this->karakterek; }
...
void Jatek::jatekos_fajl_beolvas(){
    ...
    // Fájl létezik és nem üres -> adatok beolvasása (source: https://stackoverflow.com/questions/2390912/checking-for-an-empty-file-in-c)
    if(fajl.is_open() && fajl.peek() != std::ifstream::traits_type::eof()) {
        ...
        // Ember beállítása a játékba
        this->ember->set_all(penz, megvett_r_szama, ember_nev);
...
void Jatek::licitek_inditas(){
    ...
    while(folytat != 'n'){
        ...
        // Új licitálás
        Licitalas akt_licitalas(new Raktar(this->karakterek, this->karakterek_szama), this->ember, new Robot[3]);
